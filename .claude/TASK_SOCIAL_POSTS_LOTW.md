# Sharing Links of the Week on social media

## Task
Prepare the social media posts for sharing the latest JavaFX Links of the Week. Use the following template to create engaging posts to be published on:

* Bluesky
  * Account: https://bsky.app/profile/jfxcentral.com
  * Include Frank
* Mastodon
  * Account: https://foojay.social/deck/@frankdelporte
  * Don't include Frank

Try to find the matching person link on each platform for the authors mentioned in the Links of the Week, or ask me to provide the profile link, and include them in the posts thanks section. If you can't find a matching person link, just mention their name as mentioned in the LOTW. Use the table below to keep track of the authors and their social media links, add new ones as needed.as needed.

## Template

```
Here is the new #JavaFX #LinksOfTheWeek, published on https://www.jfx-central.com/links.

Thanks to


Happy reading, and have a lovely #Java weekend!
```

Provide this as a copy-ready text for each platform, and make sure to include the appropriate hashtags and mentions. You can shorten first and last line to stay within the character limits of each platform.

## Authors

| Name | Bluesky | Mastodon |
|------|---------|----------|  
| Dirk Lemmerman | dlemmermann.bsky.social | @dlemmermann@mastodon.social |
| Gerrit Grunwald | hansolo.eu | @hansolo_@mastodon.social |
| Frank Delporte | frankdelporte.be | @frankdelporte@foojay.social |
| Erik Costlow | - | @costlow@mastodon.social |