The lib is in production ready status. That means it is tested on all operating systems (OSX, Linux, Win10) and under different threading scenarios.
...
-  One of the first lines needed is the following code to install the file handler (in the class where the _start_ method is).
...
You can also use the lib without adding the file handler. This means you can also forget the "...install" line and just load a file URL with the lib.